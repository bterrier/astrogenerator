...
QVector<double> Calculastro::coordonneesPlanetes(QDate date, QTime heure, QString planete)
{
    ...
    if(x<0) { lambda+=180;} // supprime une erreur avec l'arctangeant
    ...
    if(cos(lambda) < 0) alpha+= M_PI; // supprime une erreur avec l'arctangeant
    ...
    if(planete == "mercure") mag = 1.16 + 5*log10(E['R']*delta) + 0.02838*(i-50) + 0.0001023*pow(i-50,2.0);
    else if(planete == "venus") mag = -4.40 + 5*log10(E['R']*delta) + 0.0009*i + 0.000239*pow(i,2.0) - 0.00000065*pow(i,3.0);
    else if(planete == "mars") mag = -1.52 + 5*log10(E['R']*delta) + 0.016*i;
    else if(planete == "jupiter") mag = -9.40 + 5*log10(E['R']*delta) + 0.005*i;
    else if(planete == "saturne") mag = -8.88 + 5*log10(E['R']*delta) + 0.044*i;
    else if(planete == "uranus") mag = -7.19 + 5*log10(E['R']*delta);
    else if(planete == "neptune") mag = -6.87 + 5*log10(E['R']*delta);
    else mag = 0; // p.269
...
QMap<char,double> Calculastro::getLBR(double j2000, QString planete)
{
    ...
    if(planete == "mercure") ext = "mer";
    else if(planete == "venus") ext = "ven";
    else if(planete == "mars") ext = "mar";
    else if(planete == "jupiter") ext = "jup";
    else if(planete == "saturne") ext = "sat";
    else if(planete == "uranus") ext = "ura";
    else if(planete == "neptune") ext = "nep";
...
double Calculastro::noterObjetVisible(QString type, int interet, double magnitude, int diametre, QString niveau, double hauteurMax, int difficulte, QSettings *user)
{
    ...
    else if(type == "Amas ouverts") note += user->value("generateur/amasOuvert",NOTE_AMAS_OUVERT).toInt();
    ...
    if(niveau == "debutant") { magNiveau = magMax*0.5; numNiveau = 1; }
    if(niveau == "amateur") { magNiveau = magMax*0.6; numNiveau = 2; }
    if(niveau == "amateurc") { magNiveau = magMax*0.7; numNiveau = 3; }
    if(niveau == "expert") { magNiveau = magMax*0.8; numNiveau = 4; }
    ...
    if(noteVariable > 30) noteVariable = 30;
    else if(noteVariable < 0) noteVariable = 0;
    note += noteVariable;
